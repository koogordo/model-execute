...
class Partitioner:
    def __init__(self, bucket: str, key: str, max_partition_size: int, poke_increment: int) -> None:
        ...
        self.bucket = bucket
        self.key = key
        self.object_head = self.s3_client.head_object(Bucket=self.bucket, Key=self.key)

        self.total_size = self.object_head['ContentLength']

        self.max_partition_size = max_partition_size

        self.num_of_partitions = math.ceil(self.total_size / self.max_partition_size)

        self.estimated_partition_size = math.ceil(self.total_size / self.num_of_partitions)
    ...
    def partition(self):
        for i in range(self.num_of_partitions):
            ...
            if i == 0:
                ...
                partition.offset = self.partitions[i - 1].offset + self.partitions[i - 1].length
            ...
            if ((partition.offset + length) > self.total_size) or ((partition.offset + length + self.poke_increment) > self.total_size):
                length = (partition.offset + length) - self.total_size
            else:
                while newline_not_found:
                    poke_chunk = self.s3_client.get_object(Bucket=self.bucket, Key=self.key, Range=f'bytes={partition.offset + length}-{partition.offset + length + self.poke_increment}')
